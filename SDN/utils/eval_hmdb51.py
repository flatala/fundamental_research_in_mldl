...
class HMDBclassification(object):
    ...
    def _import_prediction(self, prediction_filename):
        """Reads prediction file, skips unknown labels, and returns
           the prediction instances."""
        with open(prediction_filename, 'r') as fobj:
            data = json.load(fobj)
        ...
        for videoid, v in data['results'].items():
            for result in v:
                ...
                # skip if label is missing or unseen in ground truth
                if lbl not in self.activity_index:
                    if self.verbose:
                        print(f"[WARN] skipping unknown label '{lbl}' for video {videoid}")
                    continue

                video_lst.append(videoid)
                label_lst.append(self.activity_index[lbl])
                score_lst.append(result['score'])
