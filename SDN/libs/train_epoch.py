...
def train_epoch(epoch, data_loader, model, criterion, optimizer, opt,
    ...
    end_time = time.time()
    for i, (inputs, targets) in enumerate(tqdm(data_loader, desc="Batches"), 0):
        data_time.update(time.time() - end_time)
...
def train_adv_msk_epoch(epoch, data_loaders, model, criterions, optimizer, opt,
    ...
    for i, ((inputs_unmasked, targets_unmasked, places_unmasked),
            (inputs_masked, targets_masked, maskings)) in enumerate(tqdm(zip(data_loader_unmasked, data_loader_masked), desc="Batches"), 0):
    # for i, ((inputs_unmasked, targets_unmasked, places_unmasked),
    #         (inputs_masked, targets_masked, maskings)) in enumerate(zip(data_loader_unmasked, data_loader_masked)):
        data_time.update(time.time() - end_time)
        ...
        if opt.is_place_adv:
            if opt.is_place_soft:
                _, places_hard = torch.max(places_unmasked, 1)
                place_acc = calculate_accuracy_pt_0_4(outputs_places_unmasked, places_hard)
            else:
                place_acc = calculate_accuracy_pt_0_4(outputs_places_unmasked, places_unmasked)
            place_losses.update(loss_place.item(), inputs_unmasked.size(0))
        else:
        ...
        # Masked forward
        print(f'num of actual masking_inds = {maskings.sum()}/{maskings.shape[0]}')
        if not opt.no_cuda:
        ...
        if opt.is_mask_entropy:
            if opt.is_mask_adv:
                msk_loss = criterions['mask_criterion'](outputs_rev_masked)
            else:
                msk_loss = criterions['mask_criterion'](outputs_masked)
        else:
    ...
    if tb_writer is not None:
        ...
        tb_writer.add_scalar('train/acc/top1_acc_place', place_accuracies.avg, epoch)
        tb_writer.add_scalar('train/msk_loss/msk_action', msk_act_losses.avg, epoch)
        tb_writer.add_scalar('train/acc/top1_msk_acc_action', msk_act_accuracies.avg, epoch)
