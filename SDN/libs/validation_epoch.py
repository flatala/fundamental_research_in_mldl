...
#         data_time.update(time.time() - end_time)
#         if not opt.no_cuda:
#             targets_unmasked = targets_unmasked.cuda(non_blocking=True)
#             places_unmasked = places_unmasked.cuda(non_blocking=True)
#             targets_masked = targets_masked.cuda(non_blocking=True)
...
def val_epoch(epoch, data_loader, model, criterion, opt, logger, tb_writer=None):
    ...
    for inputs, targets in tqdm(data_loader, desc=f"Val Epoch {epoch}", leave=False):
        data_time.update(time.time() - end_time)
...
def val_adv_epoch(epoch, data_loader, model, criterions, opt, logger, tb_writer=None):
    ...
    end_time = time.time()
    for inputs, targets, places in tqdm(data_loader, desc=f"Val Adv Epoch {epoch}", leave=False):
        data_time.update(time.time() - end_time)
        ...
        if opt.is_place_adv:
            if opt.is_place_soft:
                _, places_hard = torch.max(places, 1)
                place_acc = calculate_accuracy_pt_0_4(outputs_places, places_hard)
            else:
                place_acc = calculate_accuracy_pt_0_4(outputs_places, places)
            place_losses.update(loss_place.item(), inputs.size(0))
        else:
    ...
    print(
        f"Epoch: {epoch}\n"
        f"  Loss total   {losses.avg:.4f}\n"
        f"  Loss action  {act_losses.avg:.4f}\n"
        f"  Loss place   {place_losses.avg:.4f}\n"
        f"  Acc action   {act_accuracies.avg:.3f}\n"
        f"  Acc place    {place_accuracies.avg:.3f}\n"
    )
...
def val_adv_msk_epoch(epoch, data_loaders, model, criterions, opt, logger, tb_writer=None):
    ...
    data_loader_unmasked, data_loader_masked = data_loaders
    end_time = time.time()

    for (inputs_unmasked, targets_unmasked, places_unmasked), (inputs_masked, targets_masked, maskings) in \
            tqdm(zip(data_loader_unmasked, data_loader_masked),
                 desc=f"Val Adv Msk Epoch {epoch}", leave=False):
        data_time.update(time.time() - end_time)

        # Unmasked forward
        if not opt.no_cuda:
            inputs_unmasked = inputs_unmasked.cuda(non_blocking=True)
            targets_unmasked = targets_unmasked.cuda(non_blocking=True)
            places_unmasked = places_unmasked.cuda(non_blocking=True)
        if opt.model == 'vgg':
            inputs_unmasked = inputs_unmasked.squeeze()
        ...
        with torch.no_grad():
            if opt.is_mask_entropy and opt.is_mask_adv:
                outputs_unmasked, outputs_places_unmasked, outputs_rev_unmasked = model(inputs_unmasked)
            elif opt.is_mask_entropy:
                outputs_unmasked, outputs_places_unmasked = model(inputs_unmasked) if opt.is_place_adv else (model(inputs_unmasked), None)
            else:
                outputs_unmasked, outputs_rev_unmasked = model(inputs_unmasked)
            ...
            if opt.is_place_adv:
                ...
                if opt.is_place_entropy and epoch >= opt.warm_up_epochs:
                    loss_place_entropy = criterions['places_entropy'](outputs_places_unmasked)
                    loss += loss_place_entropy
            losses.update(loss.item(), inputs_unmasked.size(0))

            act_accuracies.update(calculate_accuracy_pt_0_4(outputs_unmasked, targets_unmasked), inputs_unmasked.size(0))
            place_accuracies.update(
                calculate_accuracy_pt_0_4(
                    outputs_places_unmasked if opt.is_place_adv else torch.zeros_like(outputs_unmasked),
                    torch.max(places_unmasked,1)[1] if (opt.is_place_adv and opt.is_place_soft) else places_unmasked
                ), inputs_unmasked.size(0)
            )

        # Masked forward
        if not opt.no_cuda:
            inputs_masked = inputs_masked.cuda(non_blocking=True)
            targets_masked = targets_masked.cuda(non_blocking=True)
        if opt.model == 'vgg':
            inputs_masked = inputs_masked.squeeze()
        ...
        with torch.no_grad():
            ...
            if opt.is_mask_entropy:
                msk_loss = criterions['mask_criterion'](outputs_rev_masked if opt.is_mask_adv else outputs_masked)
            else:
                msk_loss = criterions['mask_criterion'](outputs_rev_masked, targets_masked)

        msk_act_acc = calculate_accuracy_pt_0_4(
            outputs_rev_masked if opt.is_mask_adv else outputs_masked,
            targets_masked
        )
        msk_act_losses.update(msk_loss.item(), inputs_masked.size(0))
        msk_act_accuracies.update(msk_act_acc, inputs_masked.size(0))
